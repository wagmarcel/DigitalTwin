...
create_ngsild() {
    curl -vv -X POST -H "Authorization: Bearer $1" -d @$2 http://ngsild.local/ngsi-ld/v1/entities/ -H "Content-Type: application/ld+json" >&3
}
...
delete_ngsild() {
    curl -vv -X DELETE -H "Authorization: Bearer $1" http://ngsild.local/ngsi-ld/v1/entities/$2 -H "Content-Type: application/ld+json" >&3
}

setup() {
    (exec sudo -E kubefwd -n iff -l app.kubernetes.io/name=kafka svc) &
    echo "# launched kubefwd for kafka, wait some seconds to give kubefwd to launch the services" >&3
    sleep 10
}
teardown(){
    echo "# now killing kubefwd" >&3
...
@test "verify debezium bridge sends updates to attributes and entity topics when entity is created" {
    ...
    (exec stdbuf -oL kafkacat -C -t ${KAFKACAT_ATTRIBUTES_TOPIC} -b ${KAFKA_BOOTSTRAP} 2>&3 >${KAFKACAT_ATTRIBUTES}) &
    (exec stdbuf -oL kafkacat -C -t ${KAFKACAT_ENTITY_CUTTER_TOPIC} -b ${KAFKA_BOOTSTRAP} 2>&3 >${KAFKACAT_ENTITY_CUTTER}) &
    (exec stdbuf -oL kafkacat -C -t ${KAFKACAT_ENTITY_PLASMACUTTER_TOPIC} -b ${KAFKA_BOOTSTRAP} 2>&3 >${KAFKACAT_ENTITY_PLASMACUTTER}) &
    echo "# launched kafkacat for debezium updates, wait some time to let it connect"
    sleep 3
    password=$(get_password)
    token=$(get_token)
    create_ngsild $token $CUTTER
    echo "# Sent ngsild updates, sleep 5s to let debezium bridge react" >&3
    sleep 5
