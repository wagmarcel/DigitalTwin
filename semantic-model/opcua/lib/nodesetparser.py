...
Please set it explictly.")
    ...
    def parse(self):
        ...
        for tag_name, _ in type_nodeclasses:
            uanodes = self.root.findall(tag_name, self.xml_ns)
            # To avoid errors with missing types, we first add all type names and afterwards the full type definition
            for uanode in uanodes:
                self.add_type_name_only(uanode)
            # Now add the full type definition, which might reference other types, but since we already added all type names, this should not cause any issues
            for uanode in uanodes:
...
{event_notifier} in node {classiri}")
    ...
    def get_typedefinition_from_references(self, references, ref_classiri, node):
        ...
        if ref_id != baseObjectTypeId and ref_id != referencesId and ref_id != baseDataTypeId and\
                ref_id != baseVariableType and not mandatory_typedef_found:
