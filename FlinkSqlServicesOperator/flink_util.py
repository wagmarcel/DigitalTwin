...
timeout = os.getenv("IFF_FLINK_OPERATOR_TIMEOUT") or 5
...
def get_job_status(logger, job_id):
    ...
    job_response = requests.get(
        f"{FLINK_URL}/jobs/{job_id}", timeout=timeout)
...
def cancel_job(logger, job_id):
    ...
    response = requests.patch(f"{FLINK_URL}/jobs/{job_id}", timeout=timeout)
...
def stop_job(logger, job_id, savepoint_dir):
    ...
    job_response = requests.post(
        f"{FLINK_URL}/jobs/{job_id}/stop", json=json, timeout=timeout)
    job_response.raise_for_status()
