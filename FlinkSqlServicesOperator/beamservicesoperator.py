...
timeout = os.getenv("IFF_FLINK_OPERATOR_TIMEOUT") or 5.0
...
async def updates(stopped, patch, logger, body, spec, status, **kwargs):
    ...
    try:
        jobs = requests.get(
            f"{FLINK_URL}/jobs", timeout=timeout).json().get("jobs", [])
    ...
    try:
        with open(jarfile_path, "rb") as jarfile:
            response = requests.post(
                ...
                files={"jarfile": jarfile}, timeout=timeout)
...
def create_job(body, spec, jar_id):
    ...
    response = requests.post(f"{FLINK_URL}/jars/{jar_id}/run",
                             json={"entryClass": entry_class,
                                   "programArgs": args}, timeout=timeout)
    if response.status_code != 200:
...
def check_readiness(body):
    ...
    try:
        response = requests.get(f"{FLINK_URL}/overview", timeout=timeout)
        if response.status_code == 200:
